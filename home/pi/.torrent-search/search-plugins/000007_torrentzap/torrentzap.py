...
class TorrentzapPluginResult(TorrentSearch.Plugin.PluginResult):
   ...
   def _parseLinks(self,url):
      ...
      reflink=urllib.basejoin(url, htmltools.find_elements(tree.getRootElement(), "img", src="http://static.torrentzap.com/images/b_down.png")[0].parent.prop("href"))
      try:
...
class TorrentzapPlugin(TorrentSearch.Plugin.Plugin):
   ...
   def _run_search(self,pattern,page=1,href=None):
      ...
      for i in lines:
         try:
            date,label,links,size,seeders,leechers,health=htmltools.find_elements(i,"td",1)
