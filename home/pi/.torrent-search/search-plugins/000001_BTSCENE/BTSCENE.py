...
class BTSCENETorrentPlugin(TorrentSearch.Plugin.Plugin):
   ...
   def _run_search(self,pattern,page=1,href=None):
      if href==None:
         href="http://www.btscene.net/verified-search/torrent/%s/"%urllib.quote_plus(pattern)
      c=httplib2.Http()
      resp,content=c.request(href)
      tree=libxml2.htmlParseDoc(content,"utf-8")
      try:
         data = htmltools.find_elements(tree.getRootElement(), "div", **{'class':'index_t'})[0].getContent().rstrip().lstrip()[:-8]
         i=len(data)-1
      ...
      for i in lines:
         try:
            if i.prop('id') and i.prop('id')[0]=="_":
               link,size,seeders,leechers,updated=htmltools.find_elements(i,"td")
               date=self._parseDate(htmltools.find_elements(link, "div", **{'class':'subinfo'})[0].getContent().rstrip().lstrip())
               ...
               resp,content = c.request(urllib.basejoin(href, details_link.prop('href')))
